...
double principle_cal(double, double, int);
int main()
{
	int total_payments;
	...
	principle = principle_cal(price, annual_interest, total_payments);
	payment = payment_cal(price, annual_interest, total_payments);
	printf("\n Amount borrowed - $ %f ", principle);
	printf("\n Monthly payment - $ %.2f ", payment);
}

double payment_cal(double principle, double annual_interest, int total_payments)
{
	double payment = (annual_interest*principle)/(1 - pow((1 + annual_interest), -1*total_payments));
	return payment; 
}

double principle_cal(double price, double annual_interest, int total_payments)
{
